...
namespace Kudu.Core.Jobs
{
    public abstract class BaseJobRunner
    {
        ...
        protected void RunJobInstance(JobBase job, IJobLogger logger, string runId, string trigger, int port)
        {
            ...
            using (var jobStartedReporter = new JobStartedReporter(_analytics, job, trigger, Settings.GetWebSiteSku(), JobDataPath))
            {
                try
                {
                    ...
                    exe.EnvironmentVariables[WellKnownEnvironmentVariables.WebJobPort] = port.ToString();
